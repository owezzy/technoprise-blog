...
func New(w io.Writer, minLevel Level, serviceName string, traceIDFunc TraceIDFunc) *Logger {
	return new(w, minLevel, serviceName, traceIDFunc, Events{}, false)
}

// NewWithNoColor constructs a new log for application use with color control.
func NewWithNoColor(w io.Writer, minLevel Level, serviceName string, traceIDFunc TraceIDFunc, noColor bool) *Logger {
	return new(w, minLevel, serviceName, traceIDFunc, Events{}, noColor)
}
...
// NewWithEventsAndNoColor constructs a new log for application use with events and color control.
func NewWithEventsAndNoColor(w io.Writer, minLevel Level, serviceName string, traceIDFunc TraceIDFunc, events Events, noColor bool) *Logger {
	return new(w, minLevel, serviceName, traceIDFunc, events, noColor)
}
...
func new(w io.Writer, minLevel Level, serviceName string, traceIDFunc TraceIDFunc, events Events, noColor bool) *Logger {
	...
	// Construct the appropriate JSON handler based on color preference.
	var handler slog.Handler
	if noColor {
		// Use plain JSON handler without colors
		handler = slog.NewJSONHandler(w, &slog.HandlerOptions{AddSource: true, Level: slog.Level(minLevel), ReplaceAttr: f})
	} else {
		// Use colored JSON handler
		handler = newColoredJSONHandler(w, &slog.HandlerOptions{AddSource: true, Level: slog.Level(minLevel), ReplaceAttr: f})
	}
