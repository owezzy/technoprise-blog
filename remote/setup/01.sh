...
# Prompt to enter a password for the PostgreSQL technoprise user (rather than hard-coding
# a password in this script).

read -p "Enter password for technoprise DB user: " DB_PASSWORD
...
# Set up the technoprise DB and create a user account with the password entered earlier.
sudo -i -u postgres psql -c "CREATE DATABASE technoprise"
sudo -i -u postgres psql -d technoprise -c "CREATE EXTENSION IF NOT EXISTS citext"
sudo -i -u postgres psql -d technoprise -c "CREATE ROLE technoprise WITH LOGIN PASSWORD '${DB_PASSWORD}'"

# Add a DSN for connecting to the technoprise database to the system-wide environment
# variables in the /etc/environment file.
echo "TECHNOPRISE_DB_DSN='postgres://technoprise:${DB_PASSWORD}@localhost/technoprise'" >> /etc/environment

# Install Caddy (see https://caddyserver.com/docs/install#debian-ubuntu-raspbian).
apt install -y debian-keyring debian-archive-keyring apt-transport-https
curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/gpg.key' | sudo gpg --dearmor -o /usr/share/keyrings/caddy-stable-ar 
